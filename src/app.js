...
const UI_LOCALES = {
    tr: {
        ...
        googleUnavailable: "Google girişi için sunucu kurulumu gerekli. E-posta ile giriş yapın.",
    ...
    en: {
        ...
        googleUnavailable: "Google sign-in needs server setup. Use email login instead.",
...
// --- MERGE EXTENDED CURRICULUM into WORDS_DATA ---
// words_extra.js (loaded before app.js) provides additional chapters per level.
if (typeof WORDS_EXTRA !== 'undefined' && typeof WORDS_DATA !== 'undefined') {
    for (const level of ['Başlangıç', 'Orta', 'İleri']) {
        if (WORDS_EXTRA[level] && Array.isArray(WORDS_EXTRA[level])) {
            WORDS_DATA[level] = (WORDS_DATA[level] || []).concat(WORDS_EXTRA[level]);
        }
    }
}
...
class LinguPro {
    ...
    // Language-specific name pools for filling "..." placeholders in phrases like
    // "My name is ..." → "My name is Alex" (EN) / "Mi nombre es Carlos" (ES), etc.
    // This makes placeholder phrases feel natural and language-appropriate.
    _getLangNamePool(lang) {
        const pools = {
            en: { names: ['Alex','Emma','Jack','Olivia','Ryan','Sophia','Lucas','Mia','Ethan','Ava'], cities: ['London','New York','Sydney','Toronto','Dublin'] },
            tr: { names: ['Ali','Ayşe','Mehmet','Fatma','Ahmet','Zeynep','Emre','Elif','Burak','Merve'], cities: ['İstanbul','Ankara','İzmir','Bursa','Antalya'] },
            es: { names: ['Carlos','María','Diego','Lucía','Pablo','Sofía','Javier','Elena','Miguel','Carmen'], cities: ['Madrid','Barcelona','Sevilla','Valencia','Bogotá'] },
            fr: { names: ['Pierre','Marie','Luc','Sophie','Antoine','Camille','Louis','Léa','Hugo','Emma'], cities: ['Paris','Lyon','Marseille','Bordeaux','Nice'] },
            de: { names: ['Max','Anna','Lukas','Mia','Felix','Lena','Jonas','Emma','Paul','Hannah'], cities: ['Berlin','München','Hamburg','Köln','Frankfurt'] },
            it: { names: ['Marco','Giulia','Luca','Sofia','Andrea','Alessandro','Chiara','Francesco','Martina','Davide'], cities: ['Roma','Milano','Napoli','Firenze','Torino'] },
            pt: { names: ['João','Maria','Pedro','Ana','Rafael','Beatriz','Carlos','Camila','Lucas','Juliana'], cities: ['Lisboa','Porto','São Paulo','Rio de Janeiro','Braga'] },
            ru: { names: ['Иван','Анна','Дмитрий','Елена','Сергей','Ольга','Алексей','Мария','Николай','Татьяна'], cities: ['Москва','Санкт-Петербург','Казань','Новосибирск','Сочи'] },
            ja: { names: ['太郎','花子','健太','美咲','翔太','陽菜','蓮','結衣','大輝','葵'], cities: ['東京','大阪','京都','横浜','札幌'] },
            zh: { names: ['小明','小红','大伟','美丽','建国','丽华','文涛','小芳','志强','雅婷'], cities: ['北京','上海','广州','深圳','西安'] },
            ar: { names: ['أحمد','فاطمة','محمد','عائشة','علي','خديجة','عمر','زينب','يوسف','مريم'], cities: ['الرياض','القاهرة','دبي','بيروت','عمّان'] },
            ko: { names: ['민준','서연','도윤','서아','시우','하준','지유','주원','지호','예준'], cities: ['서울','부산','인천','대구','광주'] },
            hi: { names: ['अरुण','प्रिया','विक्रम','आनंद','नीलम','राजेश','सुनीता','गौरव','कविता','मोहन'], cities: ['दिल्ली','मुंबई','कोलकाता','चेन्नई','जयपुर'] },
            nl: { names: ['Daan','Sophie','Lucas','Emma','Sem','Lotte','Bram','Tess','Lars','Saar'], cities: ['Amsterdam','Rotterdam','Den Haag','Utrecht','Eindhoven'] },
            sv: { names: ['Erik','Anna','Lars','Emma','Johan','Karin','Anders','Lisa','Per','Eva'], cities: ['Stockholm','Göteborg','Malmö','Uppsala','Kiruna'] }
        };
        return pools[lang] || pools.en;
    }

    // Replace "..." (and trailing ellipsis) in a word/phrase with a language-
    // appropriate name. Returns the filled string. Idempotent if no placeholder.
    fillPlaceholders(text, lang) {
        if (!text || !String(text).includes('...')) return text;
        const pool = this._getLangNamePool(lang);
        const name = pool.names[Math.floor(Math.random() * pool.names.length)];
        // Replace "..." at end, middle, or standalone
        return String(text).replace(/\s*\.{2,}/g, ' ' + name).replace(/\.{2,}/g, name).trim();
    }
    ...
    async getProcessedChapter(chapter) {
        ...
        if (sL === 'tr' && tL === 'en') {
            // Fill "..." placeholders with language-specific names even in fast path
            processedChapter.words = processedChapter.words.map(w => ({
                ...w,
                en: this.fillPlaceholders(w.en, tL),
                tr: this.fillPlaceholders(w.tr, sL)
            }));
        ...
        const wordPromises = chapter.words.map(async (w) => {
            ...
            // Fill "..." placeholders with language-specific names
            finalTarget = this.fillPlaceholders(finalTarget, tL);
            finalMeaning = this.fillPlaceholders(finalMeaning, sL);
    ...
    applyLocalization() {
        ...
        // Placeholders
        // Auth form placeholders (email-based auth) + Google buttons + mobile nav
        if (document.getElementById('loginEmail')) document.getElementById('loginEmail').placeholder = L.emailPlc || 'Email';
        if (document.getElementById('loginPassword')) document.getElementById('loginPassword').placeholder = L.passwordPlc;
        if (document.getElementById('regEmail')) document.getElementById('regEmail').placeholder = L.emailPlc || 'Email';
        if (document.getElementById('regDisplayName')) document.getElementById('regDisplayName').placeholder = L.displayNamePlc || 'Display Name (Optional)';
        if (document.getElementById('regPassword')) document.getElementById('regPassword').placeholder = L.createPassPlc || L.passwordPlc;
        if (document.getElementById('regPasswordConfirm')) document.getElementById('regPasswordConfirm').placeholder = L.passConfirmPlc;
        // Google sign-in button labels
        if (document.getElementById('googleLoginBtnText')) document.getElementById('googleLoginBtnText').textContent = L.googleLoginBtn || 'Continue with Google';
        if (document.getElementById('googleRegBtnText')) document.getElementById('googleRegBtnText').textContent = L.googleLoginBtn || 'Continue with Google';
        // "Logged in as:" label in settings
        if (document.getElementById('loggedInAsLabel')) document.getElementById('loggedInAsLabel').textContent = (L.loggedInAs || 'Logged in as:') + ' ';
        // Mobile bottom navigation labels (kept in sync with the sidebar nav)
        const mobNavItems = {
            'mobNavLearn': L.navLearn,
            'mobNavPractice': L.navPractice,
            'mobNavLeagues': L.navLeagues,
            'mobNavSettings': L.navSettings
        };
        for (const [id, text] of Object.entries(mobNavItems)) {
            const el = document.getElementById(id);
            if (el && el.querySelector('span')) el.querySelector('span').textContent = text;
        }
    ...
    async renderLevels() {
        ...
        // Section-specific badge: show the target-language flag on every level card so
        // each section is visually tied to the language being learned.
        const tFlag = (LANG_FLAGS[this.settings.targetLang] && LANG_FLAGS[this.settings.targetLang].flag) || "🌐";
        const tName = (LANG_FLAGS[this.settings.targetLang])
            ? (this.settings.uiLang === 'tr' ? LANG_FLAGS[this.settings.targetLang].nameTr : LANG_FLAGS[this.settings.targetLang].nameEn)
            : "";
        ...
        levels.forEach(lvl => {
            ...
            card.innerHTML = `
                <div class="level-lang-badge" style="position:absolute;top:14px;right:16px;font-size:1.7rem;line-height:1;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.4));" title="${tName}">${tFlag}</div>
...
`;
    ...
    async nextQuestion() {
        ...
        try {
            ...
            if (qType !== 4) {
                ...
                // FINAL LANGUAGE GUARD: if the correct answer still cannot be rendered in
                // the expected language, NEVER display a wrong-language question — skip it.
                if (!correctScriptCheck) {
                    console.warn(`Skipping question: correct answer "${correctOption}" could not be rendered in ${(displayKey === 'en' ? targetLang : sourceLang)}.`);
                    this.current.index++;
                    this.current.isLocked = false;
                    this.nextQuestion();
                    return;
                ...
                if (options.length < 4) {
                    ...
                    // LANGUAGE-MIX-UP FIX: raw en/tr DB words must be translated to the
                    // expected answer language and validated before they can ever become
                    // an option. This stops e.g. a Turkish word appearing in an English
                    // (or any non-TR) quiz, or an Arabic word in a non-Arabic quiz.
                    const fbExpectedLang = (displayKey === 'en') ? targetLang : sourceLang;
                    for (const w of shuffled) {
                        if (options.length >= 4) break;
                        const wHasSpaces = (String(w.en).includes(' ') || String(w.tr).includes(' '));
                        if (wHasSpaces !== targetHasSpacesFb) continue;
                        let val = (displayKey === 'en') ? String(w.en) : String(w.tr);
                        // Translate to the expected answer language whenever it is not the
                        // native DB language of that field (en for 'en', tr for 'tr').
                        if (displayKey === 'en' && targetLang !== 'en') {
                            const t = await this.quickTranslate(val, 'en', targetLang);
                            if (!t) continue;
                            val = t;
                        } else if (displayKey === 'tr' && sourceLang !== 'tr') {
                            const t = await this.quickTranslate(val, 'tr', sourceLang);
                            if (!t) continue;
                            val = t;
                        }
                        val = this.sanitizeTranslation(String(val));
                        if (!val || val === '...' || val.includes('...') || val.length < 2) continue;
                        // STRICT language guard: reject any option in the wrong script/language
                        if (!this.isValidForLanguage(val, fbExpectedLang)) continue;
                        if (!this._scriptMatches(val, fbExpectedLang)) continue;
    ...
    normalize(str) {
        if (!str) return "";
        // Locale-agnostic lowercasing: avoids the Turkish-locale bug where English
        // "I" was turned into "ı" (dotless i), which broke dedup for non-Turkish
        // courses and allowed duplicate options to slip through.
        let s = String(str)
            .toLowerCase()
            .replace(/[.,!?;:"]/g, '')
            .replace(/\s+/g, ' ')
    ...
    // ---- LOCAL ACCOUNT SYSTEM (used when Firebase is not configured) ----
    // Lets users register / log in fully offline with email + password.
    // Accounts are stored in localStorage; progress uses the same per-user
    // storage keys as the cloud path (linguDeep_stats_<uid>).
    _getLocalAccounts() {
        try { return JSON.parse(localStorage.getItem('linguDeep_localAccounts')) || {}; }
        catch (e) { return {}; }
    }
    _saveLocalAccounts(map) {
        localStorage.setItem('linguDeep_localAccounts', JSON.stringify(map));
    }
    _hashPass(pw) {
        // Lightweight obfuscation for local-only accounts (NOT cryptographic security)
        let h = 5381;
        const s = String(pw);
        for (let i = 0; i < s.length; i++) h = ((h << 5) + h) + s.charCodeAt(i);
        return String(h >>> 0);
    }
    _localUserObject(account) {
        // Shape compatible with _handleAuthResult (uid / email / displayName /
        // emailVerified / providerData). emailVerified=true so local users are
        // never blocked by the unverified-email guard.
        return {
            uid: account.uid,
            email: account.email,
    ...
    async googleLogin(formCtx = 'login') {
        ...
        // LOCAL MODE: Google sign-in requires a configured Firebase backend
        if (typeof fbAuth === 'undefined') {
            this._setAuthError(errId, this.locale.googleUnavailable || 'Google sign-in requires server configuration. Please use email login.');
    ...
    async login() {
        ...
        // LOCAL MODE: no Firebase configured → use built-in local accounts
        if (typeof fbAuth === 'undefined') {
            const accounts = this._getLocalAccounts();
            const key = email.toLowerCase();
            const account = accounts[key];
            if (!account || account.passHash !== this._hashPass(password)) {
                this._setAuthError('login', this.locale.loginError);
                btn.disabled = false;
            ...
            btn.disabled = false;
            btn.innerHTML = origHTML;
            await this._handleAuthResult(this._localUserObject(account));
            return;
        }
    ...
    async register() {
        ...
        // LOCAL MODE: no Firebase configured → create a built-in local account
        if (typeof fbAuth === 'undefined') {
            const accounts = this._getLocalAccounts();
            const key = email.toLowerCase();
            if (accounts[key]) {
                this._setAuthError('register', this.locale.registerUsernameTaken);
                btn.disabled = false;
                btn.innerHTML = origHTML;
                return;
            }
            const account = {
                ...
                passHash: this._hashPass(password),
                emailVerified: true
            };
            ...
            btn.disabled = false;
            btn.innerHTML = origHTML;
            // Log straight in — no email verification needed in local mode
            await this._handleAuthResult(this._localUserObject(account));
            return;
        }
    ...
    saveToCloud(immediate = false) {
        if (!this.currentUser) return;
        if (typeof fbDb === 'undefined') return; // local-only mode — nothing to sync
        if (immediate) {
    ...
    async loadFromCloud() {
        if (!this.currentUser) return;
        if (typeof fbDb === 'undefined') return; // local-only mode — nothing to load
        try {
    ...
    async showPlacementQuestion() {
        ...
        if (distractors.length < 3) {
            ...
            for (const w of safetyPool) {
                ...
                if (!val) continue;
                // STRICT language guard: never inject a wrong-language option
                if (!this.isValidForLanguage(val, sourceLang)) continue;
                if (!this._scriptMatches(val, sourceLang)) continue;
                const norm = this.normalize(val);
                if (!seen.has(norm)) {
                    distractors.push(val);
                    seen.add(norm);
