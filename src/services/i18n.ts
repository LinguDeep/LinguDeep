import { LANGUAGE_VOCABULARY, INTERMEDIATE_VOCAB, ADVANCED_VOCAB } from './dbInitializer';
...
export function localizePrompt(prompt: string, interfaceLang: string): string {
  ...
  const buildVocabWords = (): Record<string, Record<string, string>> => {
    const result: Record<string, Record<string, string>> = {};
    const languages = Object.keys(LANGUAGE_VOCABULARY);
    for (const lang of languages) {
      result[lang] = {};
      
      // 1. From LANGUAGE_VOCABULARY
      const baseVocab = LANGUAGE_VOCABULARY[lang];
      const enBaseVocab = LANGUAGE_VOCABULARY.en;
      if (baseVocab && enBaseVocab) {
        for (const key of Object.keys(baseVocab) as (keyof typeof baseVocab)[]) {
          const enVal = enBaseVocab[key];
          const langVal = baseVocab[key];
          if (enVal && langVal) {
            const capEnVal = enVal.charAt(0).toUpperCase() + enVal.slice(1);
            result[lang][capEnVal] = langVal;
            result[lang][enVal] = langVal;
          }
        }
      }
      // 2. From INTERMEDIATE_VOCAB
  ...
  const vocabWords = buildVocabWords();
